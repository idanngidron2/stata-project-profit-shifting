//---------------------------------------------
//Step 4b: service debit EU import from non-EU
//---------------------------------------------

//the dofile that handles (including imputations) service debit data from non-EU (EU countries importing from non-EU countries)	


//Debit only and only non EU
...
replace bop_its6_det$temp =0 if partner==geo & partner!="EXT_EU28"

gen nongroup_partner=partner!="EU28"
gen imports=nongroup_partner*bop_its6_det$temp
gen bop_notEU=bop_its6_det$temp if (geo!="EU28" & geo!="EU27_2020")


//We begin by counting the missing "partner" observations- If all countries report which partners they import from (temp==28 if all report). We also calculate the value of imports that is unaccounted for pr. partner by using the EU aggregate
...
gen imports_no_EU28=bop_its6_det$temp if (geo!="EU28" & geo!="EU27_2020")
...
replace bop_its6_det$temp=res_EU28_percat_partner if bop_its6_det$temp==. & (geo!="EU28" & geo!="EU27_2020") & temp==27
...
//Second strictly logical (true) imputation based on all categories having to sum up to total (same logic as above: there are 12 income categories (plus an extra unknown SN category in some cases) - they all have to sum up to the total)
...
gen imports_no_EU28=bop_its6_det$temp if (geo!="EU28" & geo!="EU27_2020")
egen EU_sum_percat_partner_est=total(imports_no_EU28), by(bop_item partner) missing
gen res_EU28_percat_partner=EU_sum_percat_partner-EU_sum_percat_partner_est
...
//Second strictly logical (true) imputation based on EU28 sum and country aggregate

gen import_no_S=bop_its6_det$temp if bop_item!="S"
gen service_sum_aux=bop_its6_det$temp if countvar==1
egen service_sum = total(service_sum_aux), by(geo partner) missing
egen S_est=total(import_no_S), by(geo partner) missing
gen res_S_partner=service_sum-S_est
...
//We now attempt to bound the unreported imports by finding the minimum of all possible strictly logical maximas

//Biggest max: total imports less whatever is reported in rest of world

gen totalimports_aux= bop_its6_det$temp if partner=="EXT_EU28" & bop_item=="S"
...
//What is missing from each reporter (geo) to the entire outside world - This will give us a maximum amount of imports to be imputed pr. reporter

gen internal_sum_agg_aux=bop_its6_det$temp if partner=="EXT_EU28"
gen temp1=1 if partner=="EXT_EU28" & bop_its6_det$temp==.
...
//We estimate the import value that is accounted for by aggregating all the reports of each individual country (within categories)

egen extra_EU_est=total(imports), by(geo bop_item)

//We then calculate the difference between the sum of imports from rest of the world and the sum of individual reports - this residual is the imports that needs to be imputed pr. reporter-category (geo- bop item) 
...
//We now compute that minimum maximum by taking the minimum of all the strictly logical maximas

egen minof3max=rmin(rest service_sum totalimports extra_EU res_S_partner res_EU28_percat_partner res_EXT_EU_geo)



//how much is left when maximum of others is used for each partner-geo => this creates a minimum value of imports pr geo-partner-category observation
...
gen max_for_table= minof3max_cons if (geo!="EU28" & geo!="EU27_2020")
gen unaccounted_for_table=res_EU28_percat_partner if (geo!="EU28" & geo!="EU27_2020")
gen min_for_table= maxofmin_eu3 if (geo!="EU28" & geo!="EU27_2020")
replace min_for_table=bop_its6_det$temp if bop_its6_det$temp!=.
replace max_for_table=bop_its6_det$temp if bop_its6_det$temp!=.



//Within the bounds of min/max we will allocate the missing imports according to the missing imports of each country

gen min_aux=min_for_table if bop_missing
egen accountedfor_aux =sum(min_aux), by(partner bop_item)
gen unaccounted=res_EU28_percat_partner - accountedfor_aux //this is what needs to be allocated to reporters (geo) in the following)*/
...
gen imputed1=min_for_table+(service_sum/EUimportsfrompartner)*unaccounted if bop_missing & (geo!="EU28" & geo!="EU27_2020")
gen extra_value_aux=(service_sum/EUimportsfrompartner)*unaccounted if bop_missing & (geo!="EU28" & geo!="EU27_2020")
...
gen totalimports_aux=totalimports if (geo!="EU28" & geo!="EU27_2020") & aux==0
...
egen imputed_aux=rowtotal(imputed1 imputed2) if max_reached==0 & (imputed1!=.|imputed2!=.) & partner!="EXT_EU28", missing
...
gen check_aux=imputed if (geo!="EU28" & geo!="EU27_2020")
...
gen haven=(partner=="OFFSHO"|partner=="CH")

preserve
keep if bop_item=="S" & partner=="EXT_EU28" & geo!="EU28"
keep geo_label bop_its6_det$temp imputed min_for_table max_for_table bop_missing partner
save "$work/serviceimportsnoneu_$temp", replace
restore
...
preserve
keep if dodgy & geo!="EU28" & partner=="EXT_EU28"
keep geo_label bop_its6_det$temp imputed min_for_table max_for_table bop_missing partner
save "$work/overview_debit_nonEU_$temp", replace
restore



//Breaking down total service exports by haven


foreach name in "OFFSHO" "CH" "EXT_EU28" {
	preserve
	keep if bop_item=="S" & geo!="EU28" & partner=="`name'"
	keep geo_label bop_its6_det$temp imputed bop_missing
	gen partner_name= "`name'"
	save "$work/debit_nonEU_$temp`name'", replace
	restore
	}
...
preserve
keep if bop_item=="S" & partner!="EXT_EU28" & geo!="EU28"
keep geo_label partner imputed
save "$work/debit_nonEU_other_partners_$temp", replace
restore


preserve
keep if bop_item=="S" & haven & geo!="EU28"
keep geo_label partner_l imputed 
save "$work/debit_nonEU_Havens_$temp", replace
restore




//Creating help variable to limit dodgy (high risk payments) to total imputed services
...
//third: there can't be going more to haven than service_sum (imports from partner), total imports, or everything extra

egen max_dodgy_pr_partner=rmin(service_sum totalimports rest dodgy_extra_EU)
...
egen imputed_aux2=sum(imputed_aux) if bop_item=="SH" & partner!="EXT_EU28", by(partner bop_item)
...
//correction loop to ensure convergence
...
gen imputed_aux=dodgy_imputed if max_met==0 & extra_imputed!=0 & partner!="EXT_EU28" & dodgy_missing>0 & dodgy_missing!=.
...
preserve
keep if bop_item=="SH" & partner!="EXT_EU28" & geo!="EU28"
keep geo_label partner dodgy_imputed
save "$work/debitdodgy_nonEU_otherpartners$temp", replace
restore

preserve
keep if bop_item=="SH" & haven & partner!="EXT_EU28" & geo!="EU28"
keep geo_l partner_l dodgy_imputed
save "$work/debitdodgy_nonEU_havens$temp", replace
restore


keep if haven & bop_item=="SH"
...
//total dodgy exports to havens and breakdown
*tabout geo_l using overview_debit_nonEU.xls if haven & partner!="EXT_EU28" & geo!="EU28", append  sum c(sum dodgy_nonimputed sum dodgy_imputed sum min sum max sum dodgy_missing) h2(Dodgy services: partner haven)

foreach name in "OFFSHO" "CH" {
preserve 
keep if bop_item=="SH" & partner=="`name'" & partner!="EXT_EU28"
keep geo_l dodgy_nonimputed dodgy_imputed max min dodgy_missing partner
save "$work/debitdodgy_nonEU_$temp`name'", replace
restore
}


/*Adding lines to export into common raw data file 		//IG change
import delimited breakdown_debit_nonEU.txt, clear
export excel using "$Rawdata", sheet ("breakdown_debit_nonEU", replace)

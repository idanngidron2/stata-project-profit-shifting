//---------------------------------------------
//Step 4c: service credit EU export to non-EU
//---------------------------------------------

// the dofile that handles (including imputations) service credit data from non-EU (EU countries exporting to non-EU countries)

//credit only and only non EU
...
gen exports_no_EU28=bop_its6_det`temp' if (geo!="EU28" & geo!="EU27_2020")
...
gen exports_no_EU28=bop_its6_det`temp' if (geo!="EU28" & geo!="EU27_2020")
egen EU_sum_percat_partner_est=total(exports_no_EU28), by(bop_item partner) missing
gen res_EU28_percat_partner=EU_sum_percat_partner-EU_sum_percat_partner_est
...
gen service_sum_aux=bop_its6_det`temp' if countvar==1
egen service_sum = total(service_sum_aux), by(geo partner) missing
egen S_est=total(export_no_S), by(geo partner) missing
gen res_S_partner=service_sum-S_est
...
//Biggest max: total exports less whatever is reported in rest of world

gen totalexports_aux= bop_its6_det`temp' if partner=="EXT_EU28" & bop_item=="S"
...
gen imputed1=min_for_table+(service_sum/EUexportsfrompartner)*unaccounted if bop_missing & (geo!="EU28" & geo!="EU27_2020")
gen extra_value_aux=(service_sum/EUexportsfrompartner)*unaccounted if bop_missing & (geo!="EU28" & geo!="EU27_2020")
...
gen totalexports_aux=totalexports if (geo!="EU28" & geo!="EU27_2020") & aux==0
...
preserve
keep if bop_item=="S" & partner=="EXT_EU28" & (geo!="EU28" & geo!="EU27_2020")
keep geo_l bop_its6_det`temp' imputed min_for_table max_for_table bop_missing partner
save "$work/serviceexportsnoneu_$temp", replace
restore



*total services to Havens
*tabout geo_l using overview_credit_nonEU.xls if bop_item=="S" & haven & geo!="EU28", append  sum c(sum bop_its6_det`temp' sum imputed sum min_for_table sum max_for_table sum bop_missing) h2(Total services: partner non-EU haven)



*tototal services from ext EU28
*tabout geo_l using overview_credit_nonEU.xls if bop_item=="S" & geo!="EU28" & partner=="EXT_EU28", append  sum c(sum bop_its6_det`temp' sum imputed sum min_for_table sum max_for_table sum bop_missing) h2(Total services: partner non-EU)
...
*tabout geo_l using overview_credit_nonEU.xls if dodgy & geo!="EU28" & partner=="EXT_EU28", append  sum c(sum bop_its6_det`temp' sum imputed sum min_for_table sum max_for_table sum bop_missing) h2(Dodgy services: partner non-EU)
...
preserve
keep if bop_item=="S" & (geo!="EU28" & geo!="EU27_2020") & partner=="`name'"
keep geo_l bop_its6_det`temp' imputed bop_missing partner
save "$work/credit_nonEU_$temp`name'", replace
restore
}
...
preserve
keep if bop_item=="S" & partner!="EXT_EU28" & (geo!="EU28" & geo!="EU27_2020")
keep geo_l partner imputed
save "$work/credit_nonEU_$tempotherpartners", replace
restore

preserve
keep if bop_item=="S" & haven & (geo!="EU28" & geo!="EU27_2020")
keep geo_l partner_l imputed
save "$work/credit_nonEU_$temphavens", replace
restore
...
egen check_sum=total(check_aux), by(partner bop_item)
gen check_aux2=dodgy_imputed if (geo!="EU28" & geo!="EU27_2020")
...
preserve
keep if bop_item=="SH" & partner!="EXT_EU28" & (geo!="EU28" & geo!="EU27_2020")
keep geo_l partner dodgy_imputed
save "$work/creditdodgy_nonEU__OP$temp", replace
restore 

preserve 
keep if bop_item=="SH" & haven & partner!="EXT_EU28" & (geo!="EU28" & geo!="EU27_2020")
keep geo_l partner_l dodgy_imputed
save "$work/creditdodgy_nonEU_HA$temp", replace
restore
...
keep geo_l dodgy_nonimputed dodgy_imputed min max dodgy_missing partner
save "$work/creditnonEU_dodgytotal_$temp_`name'", replace
restore
}


gen EUhaven=1 if (geo=="MT"|geo=="CY"|geo=="BE"|geo=="IE"|geo=="LU"|geo=="NL")

preserve
*keep if bop_item=="SH" & EUhaven==1
keep if bop_item == "SH"
keep geo_l partner_l dodgy_imputed
save "$work/ser_other_co_loss_nonEU_$temp", replace
restore
...
preserve
keep if bop_item=="SH" & haven & partner!="EXT_EU28" & (geo!="EU28" & geo!="EU27_2020")
keep geo_l dodgy_nonimputed dodgy_imputed dodgy_missing partner
save "$work/overv_credit_partnerhaven_$temp", replace
restore


preserve
keep if bop_item=="SH" & partner=="EXT_EU28" & (geo!="EU28" & geo!="EU27_2020")
keep geo_l partner dodgy_nonimputed dodgy_imputed dodgy_missing
save "$work/overv_credit_partnernonEU_$temp", replace
restore
